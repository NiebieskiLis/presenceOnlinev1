...
public class AccountantPlatform {
                ...
                if (lock != null) {
                    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                        ...
                        for (String line : allList) {
                            ...
                            if (line.contains(";")) {
                                object1.addToList(new FullTimeWorker(pola[3], pola[0], pola[1], Integer.parseInt(pola[2]), 0, true, true, 0 , null));
                            }
                        }
                        try {
                ...
                if(lock != null){
                    ...
                    while ((line = br.readLine())!=null){
                        ...
                        if (line.contains(";") & pola.length>3) {
                            object1.addToList(new FullTimeWorker(pola[3], pola[0], pola[1],  pola[1], 0, 1, true, true, 1,null));
                        }
    ...
    public void menu (EntityManager em){
        ...
        WorkersList workers =  new WorkersList();
        workers.workers.addAll(query.getResultList());
        String queryString2 = "SELECT p FROM PartTimeWorker p";
        Query query2 = em.createQuery(queryString2);
        workers.workers.addAll(query2.getResultList());
        boolean flag = true;
        //A main menu made in a loop
        while (flag){
            System.out.print("What would you like to do? \n s - save all workers list  \n l - load all workers list\n p - print list \n e - exit \n ");
            String element = "";
            try {
                BufferedReader obj = new BufferedReader(new InputStreamReader(System.in));
            ...
            {
                this.SaveToFile(workers);
            }
            else if ("l".equals(element))
            {
                workers = this.readFromFile();
            }
            else {
